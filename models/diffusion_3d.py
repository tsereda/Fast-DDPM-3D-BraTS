"""
3D Fast-DDPM Model - Converted from 2D Fast-DDPM for BraTS
Critical missing component for your repository
"""
import math
import torch
...
def get_timestep_embedding(timesteps, embedding_dim):
    """Same as 2D - timestep embedding doesn't change"""
...
def Normalize(in_channels):
    # GroupNorm works the same for 3D
    return torch.nn.GroupNorm(num_groups=32, num_channels=in_channels, eps=1e-6, affine=True)


class Upsample3D(nn.Module):
    """3D Upsample - key conversion from 2D"""
...
class Downsample3D(nn.Module):
    """3D Downsample - key conversion from 2D"""
    def __init__(self, in_channels, with_conv):
        super().__init__()
        self.with_conv = with_conv
        if self.with_conv:
            self.conv = torch.nn.Conv3d(in_channels,
                                        ...
                                        kernel_size=3,
                                        stride=2,
                                        padding=1)  # Simplified padding for 3D
...
class ResnetBlock3D(nn.Module):
    """3D ResNet Block - core conversion"""
    ...
    def forward(self, x, temb):
        ...
        # Add timestep embedding - same logic as 2D
...
class AttnBlock3D(nn.Module):
    """3D Attention Block - memory-efficient for 3D volumes"""
    ...
    def forward(self, x):
        ...
        # compute attention for 3D - flatten spatial dimensions
        b, c, h, w, d = q.shape
...
class Model3D(nn.Module):
    """3D Fast-DDPM Model for BraTS - Unified 4→4 Architecture"""
    def __init__(self, config):
        ...
        volume_size = config.data.volume_size if hasattr(config.data, 'volume_size') else [64, 64, 64]
        ...
        curr_res = min(volume_size)  # Use minimum dimension for resolution tracking
    ...
    def forward(self, x, t):
        ...
        return h  # [B, 1, H, W, D] - single output modality


# For backward compatibility - alias the main model
Model = Model3D